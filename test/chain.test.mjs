...
// ⭐ Two api ids, because the buyer signs against two contracts: the protocol
// for the purchase, and the exchange token for the approval before it. Each is
// looked up under its own address, so an id registered for one is not an id for
// the other, and a bare id is not enough for either.
...
test("each api id is nested under its own contract address and the method", () => {
  const config = resolveProtocolConfig(env());
...
// already been produced and only the missing contract's relay fails.
